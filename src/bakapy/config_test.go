...
func TestParseConfig_JobsWithRemoteFilters(t *testing.T) {
	...
	if value := config.Jobs["xxx"].RemoteFilters[0]["gzip"]["level"]; value != "1" {
		t.Fatal("Remote filters does not parse values properly")
	}
}
...
func TestParseConfig_JobsWithLocalFilters(t *testing.T) {
	...
	if value := config.Jobs["zzz"].LocalFilters[0]["gzip"]["level"]; value != "3" {
